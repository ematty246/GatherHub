...
function NationalWaterDevelopmentAgency() {
  ...
  const handleAuth = async (endpoint) => {
    ...
    const response = await fetch(`${API_BASE}/${isLogin ? 'login' : 'register'}/NationalWaterDevelopmentAgency`, {
      method: 'POST',
  ...
  const fetchReports = async () => {
    const response = await fetch(`http://192.168.125.188:5000/reports/NationalWaterDevelopmentAgency`);
    const data = await response.json();
    setReports(data);
  };
...
export default NationalWaterDevelopmentAgency;