...
function FireServicesDepartment() {
  ...
  const handleAuth = async (endpoint) => {
    ...
    const response = await fetch(`${API_BASE}/${isLogin ? 'login' : 'register'}/FireServicesDepartment`, {
      method: 'POST',
  ...
  const fetchReports = async () => {
    const response = await fetch(`http://192.168.125.188:5000/reports/FireServicesDepartment`);
    const data = await response.json();
    setReports(data);
  };
...
export default FireServicesDepartment;