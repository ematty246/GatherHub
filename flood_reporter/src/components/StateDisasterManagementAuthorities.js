...
function StateDisasterManagementAuthorities() {
  ...
  const handleAuth = async (endpoint) => {
    ...
    const response = await fetch(`${API_BASE}/${isLogin ? 'login' : 'register'}/StateDisasterManagementAuthorities`, {
      method: 'POST',
  ...
  const fetchReports = async () => {
    const response = await fetch(`http://192.168.125.188:5000/reports/StateDisasterManagementAuthorities`);
    const data = await response.json();
    setReports(data);
  };
...
export default StateDisasterManagementAuthorities;
